...
| ROUTE_RECREATE_VM       | "/recreate_vm"                         | The Cloud Run path invoked by a runner VM's shutdown script when the VM dies without ever accepting a job (e.g. SPOT preemption before pickup). Re-enqueues a "create runner" Cloud Task so the lost capacity is replaced. If set to "" the recreate mechanism is disabled and such jobs stay queued forever.   |
| RUNNER_JOB_LOG_PATTERN  | "Running job:"                         | The journald log substring that marks "this runner accepted a job". Baked into each VM's recreate metadata so the shutdown script can decide whether a recreate callback is needed. Must match the Terraform variable `runner_job_log_pattern` used by the startup script's dispatch wait.                      |
...
| ZONE_BENCH_MIN_VMS      | "3"                                    | Zone circuit breaker: a zone is tried last when at least this many of its runner VMs logged an outbound dial timeout (the Ops Agent's `Exporting failed ... i/o timeout` syslog line) in the last ZONE_HEALTH_WINDOW seconds. Read from Cloud Logging on the create path, cached 60 s, fails open. Needs `logging.logEntries.list` and a runner image that ships the Ops Agent (its syslog receiver is the signal). "0" disables the breaker. |
| ZONE_BENCH_MIN_RATIO    | "0.2"                                  | Zone circuit breaker: the failing VMs must also be at least this fraction of the VMs the autoscaler created in that zone in the window.                                                                                                              |
