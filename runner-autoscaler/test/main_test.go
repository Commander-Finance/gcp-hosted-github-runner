...
func TestParseLabelGroups(t *testing.T) {
	...
	cases := []struct {
		...
		{"two single-label groups", "builder;builder-large", [][]string{{"builder"}, {"builder-large"}}},
		{"two two-label groups", "builder,linux;builder-large,linux", [][]string{{"builder", "linux"}, {"builder-large", "linux"}}},
		{"whitespace trimmed", "builder, linux ; builder-large", [][]string{{"builder", "linux"}, {"builder-large"}}},
		{"empty labels dropped", "builder,;,builder-large", [][]string{{"builder"}, {"builder-large"}}},
		{"empty groups dropped", "builder;;builder-large", [][]string{{"builder"}, {"builder-large"}}},
		{"trailing separators", "builder;", [][]string{{"builder"}}},
		{"empty string", "", [][]string{}},
...
func TestHasAnyLabelGroup(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			...
			groups:  [][]string{{"builder", "gce-machine-c2d-standard-16"}},
			wantOk:  true,
			wantMsg: "",
		},
		{
			...
			labels:  []string{"builder"},
			groups:  [][]string{{"builder"}, {"builder-large"}},
			wantOk:  true,
			wantMsg: "",
		},
		{
			name:    "multi group second matches",
			labels:  []string{"builder-large", "gce-machine-t2d-standard-4"},
			groups:  [][]string{{"builder"}, {"builder-large"}},
			wantOk:  true,
			wantMsg: "",
		},
		{
			...
			groups:  [][]string{{"builder"}, {"builder-large"}},
			wantOk:  false,
			wantMsg: "none of the label groups matched (required one of: [builder], [builder-large])",
		},
		{
			name:    "multi group magic labels skipped per group",
			labels:  []string{"builder-large", "gce-machine-t2d-standard-4"},
			groups:  [][]string{{"builder", "gce-machine-c2d-standard-16"}, {"builder-large", "gce-machine-t2d-standard-4"}},
			wantOk:  true,
			wantMsg: "",
		},
		{
		...
		},
		{
			...
			labels:  []string{"builder", "anything"},
		...
		},
		{
			...
			labels:  []string{"builder-large"},
			groups:  [][]string{{"gce-machine-c2d-standard-16"}, {"builder-large"}},
			wantOk:  true,
			wantMsg: "",
		},
		{
			...
			labels:  []string{"ghost"},
			groups:  [][]string{{"gce-machine-c2d-standard-16"}, {"builder"}},
			wantOk:  false,
			wantMsg: `missing the label(s) "builder"`,
		},
		{
			name:    "multi group miss strips magic labels from required-one-of render",
			labels:  []string{"ghost"},
			groups:  [][]string{{"builder", "gce-machine-c2d-standard-16"}, {"builder-large"}},
			wantOk:  false,
			wantMsg: "none of the label groups matched (required one of: [builder], [builder-large])",
		},
	}
